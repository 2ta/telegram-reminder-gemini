    ...
    if message_type == "text" or message_type == "voice":
        ...
        if not settings.GEMINI_API_KEY:
            ...
            try:
                ...
                prompt_template = f"""You are an intelligent assistant for detecting user intent from English text (if the user give the prompt in other language, apply all the rules below to the prompt in English and understand the user intent and pass the resualt for task in the same language of the user that used and  provide time_str and date_str in English - if the user provide date in the Jalali calendar to the Gregorian calendar).
